...
function Cvsection({
  ...
  const handleDialogControl = () => {
    if (addsSimalarFields) {
      ...
      if (dynamicFields[heading]?.length) {
        index = dynamicFields[heading].length / defaultFields.length + index;
      }
  ...
  const handleAddField = () => {
    if (dialogValuesObj.fieldName.trim() && dialogValuesObj.type) {
      ...
      const defaultDataHasField = defaultFields.some(
        (defaultName) => defaultName.name.toLowerCase() === name.toLowerCase()
      );
  ...
  const handleBlur = (e) => {
    ...
    } else if (type !== "tel" || (type !== "email" && !e.target.dataset.link))
      setCanSubmit(true);
  ...
  const checkEmptyField = (fieldObj, type, name) => {
    if (fieldObj[type]) {
      if (Object.keys(fieldObj[type]).length > 1) {
        ...
        if (Object.keys(fieldObj[type])[0] === name) delete fieldObj[type];
      }
    }
  };
            ...
            <div className="form-container">
              <div className="input-field">
                ...
                {dynamicFields[activeCvSection.name] &&
                  dynamicFields[activeCvSection.name].map((field) => {
                    if (field.type === "textarea") {
                      return (
                        <TextArea
                          key={field.id}
                ...
                <div className="dialog-overlay">
                  <div className="dialog-content">
                    <div className="select-input-type">
                      <p className="error-text">Field already exist</p>
